...
struct ProcessRowTests {
    ...
    func codable() throws {
        let original = row(pid: 42, name: "MacTools", cpu: 12.5, cpuNanoseconds: 99, memory: 1_048_576)
...
struct ProcessMergeTests {
    ...
    func fillsGaps() {
        let local = [
            ...
            row(pid: 500, cpu: 4, cpuNanoseconds: 40, memory: 2_048, name: "MacTools"),
        ]
        let privileged = [
            ...
            row(pid: 500, cpu: 99, cpuNanoseconds: 990, memory: 1, name: "MacTools"),
        ]
...
struct ProcessIdentityCacheTests {
    private func cached(
        command: String = "MacTools",
        start: UInt64? = 1_000,
        path: String? = "/Applications/MacTools.app/Contents/MacOS/MacTools",
        name: String = "MacTools"
    ...
    func reusesTheSameProcess() {
        ...
        #expect(reused?.name == "MacTools")
        #expect(reused?.path == "/Applications/MacTools.app/Contents/MacOS/MacTools")
    }
    ...
    func noCache() {
        #expect(
            ProcessSampler.reusableIdentity(cached: nil, command: "MacTools", startAbsoluteTime: 1) == nil
        )
    }
