...
func assertionWithoutTimeout() {
    ...
    #expect(request.name == "MacTools Keep Awake")
    #expect(request.details == "MacTools keeps this Mac awake until you turn it off.")
}
...
func assertionWithTimeout() {
    ...
    #expect(request.details == "MacTools keeps this Mac and its display awake for 2 hours.")
}

@Test("the app name goes into what the user reads")
func assertionName() {
    let request = AssertionRequest.make(duration: .minutes(30), keepDisplayOn: false, appName: "mactoolsctl")
    #expect(request.name == "mactoolsctl Keep Awake")
    #expect(request.details == "mactoolsctl keeps this Mac awake for 30 minutes.")
}
...
@Test(
    ...
    arguments: [
        ...
        (false, true, false, "This Mac cannot sleep at all - set with pmset, not by MacTools"),
    ]
)
