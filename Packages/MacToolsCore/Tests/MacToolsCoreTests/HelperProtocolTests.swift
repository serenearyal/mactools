...
@Test("the client requirement accepts the app and mactoolsctl, the helper one only the helper")
func requirementsNameTheRightIdentifiers() {
    let client = HelperConstants.clientCodeSigningRequirement
    #expect(client.contains("identifier \"com.serenearyal.mactools\""))
    #expect(client.contains("identifier \"com.serenearyal.mactools.mactoolsctl\""))
    #expect(client.hasPrefix("(identifier"))
    #expect(!client.contains("com.serenearyal.mactools.helper"))

    let helper = HelperConstants.helperCodeSigningRequirement
    #expect(helper.hasPrefix("identifier \"com.serenearyal.mactools.helper\""))
}
