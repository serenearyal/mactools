...
/// left on disk that admits Vent is the one holding it.
...
///    moment the last of them releases it or disconnects. A `ventctl` that
...
public final class SleepDisabledGovernor: Sendable {
    ...
    /// that outlived its flag would let Vent clear a flag the user sets by
    /// hand later.
    ...
    /// `pmset` and set it again by hand, and Vent would then clear a flag that
    ...
    private func lockedReport() -> SleepDisabledReport? {
        ...
        guard marker.isMarked else { return SleepDisabledReport(isSet: isSet, setByVent: false) }
        guard isSet else {
        ...
        return SleepDisabledReport(isSet: true, setByVent: true)
    }
