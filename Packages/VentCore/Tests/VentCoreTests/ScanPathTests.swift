...
struct FullDiskAccessTests {
    ...
    func missingFile() {
        #expect(FullDiskAccess.probe("/tmp/vent-does-not-exist-\(UUID().uuidString)") == .absent)
    }

    @Test("a readable file is granted")
    func readableFile() throws {
        let url = URL(filePath: NSTemporaryDirectory()).appending(path: "vent-fda-\(UUID().uuidString)")
