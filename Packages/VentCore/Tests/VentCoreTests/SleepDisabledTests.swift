...
struct SleepDisabledTests {
    ...
    /// on the way out: the user clears the flag by hand, Vent reads the state
    /// once, the user sets it again by hand, and Vent's own release must then
    /// leave that flag exactly where it is.
    @Test("a flag the user sets again by hand after a read is never cleared by Vent")
    func aReadDropsTheMarkerSoALaterForeignFlagSurvives() {
        ...
        #expect(power.writes == 1, "only Vent's own write ever happened")
    }
    ...
    /// R-10: `ventctl awake lid on` must not leave the flag set after it
    /// exits, and `ventctl awake lid off` must not turn the app's hold off.
    ...
    func twoClientsHoldSeparately() {
        ...
        // ventctl exits: its connection drops, and only its own hold with it.
    ...
    /// makes a `ventctl` killed mid-command safe.
    ...
    func fileMarkerRoundTrip() throws {
        let directory = URL.temporaryDirectory.appending(path: "vent-marker-\(UUID().uuidString)")
