...
final class HelperXPCTests: XCTestCase {
    ...
    private func proxy() throws -> any MacToolsHelperProtocol {
        let raw = connection.remoteObjectProxyWithErrorHandler { error in
            XCTFail("XPC connection error: \(error)")
        }
        return try XCTUnwrap(raw as? any MacToolsHelperProtocol)
    }
...
final class HelperSleepXPCTests: XCTestCase {
    ...
    /// Another client of the same helper: the app and `mactoolsctl` at once, which
    /// is the case the per-connection hold exists for.
    private func connect() -> NSXPCConnection {
        let fresh = NSXPCConnection(listenerEndpoint: listener.endpoint)
        fresh.remoteObjectInterface = NSXPCInterface(with: MacToolsHelperProtocol.self)
        fresh.resume()
        return fresh
    }
    ...
    private func proxy(on link: NSXPCConnection? = nil) throws -> any MacToolsHelperProtocol {
    ...
    func testTheFlagCrossesTheWireAndComesBackOff() throws {
        ...
        XCTAssertEqual(try report(), SleepDisabledReport(isSet: true, setByMacTools: true))
        XCTAssertEqual(try report().owner, .macTools)
    ...
    /// The user's own `pmset disablesleep 1`. MacTools reports it and stops there.
    ...
    /// before this one set it and died. Over the wire the flag is MacTools', and
    /// MacTools is the one that may take it off.
    func testAFlagLeftMarkedByAPreviousRunIsMacToolsToClear() throws {
    ...
    /// R-10 over a real pair of connections: `mactoolsctl awake lid on` exits and
    ...
    func testAHoldBelongsToTheConnectionThatTookIt() throws {
        ...
        // mactoolsctl exits, and the helper notices before anything is asserted.
        cli.invalidate()
    ...
    /// The other half of R-10: `mactoolsctl awake lid off` gives up its own hold
    ...
    /// is what makes a `kill -9` of MacTools safe, and it is the reason a Mac
    ...
    func testAConnectionThatNeverHeldTheFlagTakesNothingWithIt() throws {
        ...
        XCTAssertNil(try set(true))
        XCTAssertEqual(try report(on: watcher).owner, .macTools)
