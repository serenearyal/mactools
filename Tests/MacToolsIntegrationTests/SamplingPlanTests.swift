...
struct ProcessRowsTests {
    ...
    private var sample: [ProcessTableRow] {
        [
            ...
            row(pid: 3, name: "MacTools", cpu: 0.3, memory: 40_000_000),
    ...
    func derives() {
        ...
        #expect(rows.visible.map(\.name) == ["Xcode", "kernel_task", "launchd", "MacTools"])
    ...
    func filters() {
        ...
        #expect(mine.visible.map(\.name) == ["MacTools", "Xcode"])
