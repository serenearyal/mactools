...
let usage = """
mactoolsctl - debug CLI for MacTools

usage: mactoolsctl <command> [options]
...
'fan-set' lasts only as long as the command itself unless MacTools is running.
...
'report files' reads the cache the app and 'mactoolsctl scan' write; it never
starts a scan of its own.
"""

func fail(_ message: String) -> Never {
    FileHandle.standardError.write(Data("mactoolsctl: \(message)\n".utf8))
    exit(1)
}
...
do {
    ...
    case "helper-read":
        guard tail.count == 1 else {
            throw CLIError("'helper-read' takes one SMC key, for example 'mactoolsctl helper-read F0Ac'")
        }
    ...
    case "window":
        guard tail == ["list"] else {
            throw CLIError("'window' takes one subcommand: 'mactoolsctl window list'")
        }
    ...
    case "fan-set":
        ...
        guard positional.count == 2, let index = Int(positional[0]), let rpm = Int(positional[1]) else {
            throw CLIError("'fan-set' takes a fan index and a speed, for example 'mactoolsctl fan-set 0 2500 --hold'")
        }
    ...
    case "-h", "--help", "help":
        print(usage)
    default:
        FileHandle.standardError.write(Data("mactoolsctl: unknown command '\(command)'\n\n".utf8))
