...
/// `mactoolsctl awake`: the read side of Keep Awake, and a hold that takes the
...
enum AwakeCommands {
    ...
    ///
    /// The name says "mactoolsctl" rather than "MacTools" so a `pmset -g assertions`
    ...
    static func hold(minutes: Int) throws {
        ...
        print("check with: pmset -g assertions | grep -i mactools")
    ...
    private static func ownership() -> String {
        do {
            ...
            case .macTools: "set by        MacTools' helper, which clears it when the last client holding it goes"
            case .somebodyElse: "set by        somebody else (pmset); MacTools leaves it alone"
            }
        } catch {
    ...
    /// `mactoolsctl awake lid on|off`: the same helper call the app's switch makes.
    ///
