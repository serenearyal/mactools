...
/// `mactoolsctl report processes|files`: the same text the app's Copy for AI puts
...
enum ReportCommands {
    ...
    /// whole-volume walk is minutes, and `mactoolsctl scan` is the command for it.
    static func files(includePreamble: Bool, format: TableFormat, limit: Int) throws {
        ...
        guard let result = try cache.load(volume: ScanCache.volumeIdentifier(for: root)) else {
            throw CLIError(
                "no cached scan for this volume; run 'mactoolsctl scan' or scan once in the app"
            )
        }
