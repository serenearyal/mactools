...
///
/// `mactoolsctl` is signed with the same identity as the app and its bundle
...
enum HelperCommands {
    static func ping() throws {
        ...
        print(answer)
        print("client: \(HelperConstants.mactoolsctlBundleIdentifier)")
    ...
    /// The system-wide `SleepDisabled` flag and whether MacTools' helper is the
    ...
    /// `mactoolsctl` is a synchronous tool: every command runs to the end and
