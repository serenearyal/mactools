...
enum FanCommands {
    ...
    /// The one sentence that explains why `mactoolsctl fan-set` on its own looks
    /// like it did nothing.
    private static func lifetimeNote(index: Int, rpm: Int) -> String {
        if isAppRunning {
            return """
                note: MacTools is running and holds a connection, so the helper keeps the \
                mode; MacTools writes its own stored mode back within seconds, though. \
        ...
        return """
            ...
            'mactoolsctl fan-set \(index) \(rpm) --hold' to hold it, or set the mode in \
            MacTools' Fans tab, which keeps a connection open.
            """
    }
