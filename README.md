...
- **Recovery runs at boot.** `SleepDisabled` survives a reboot, so the daemon plist carries `RunAtLoad`: the helper starts with the machine, finds a marker from a run that was killed, and clears the flag before anybody opens Vent. It then waits on its XPC listener and polls nothing.
- **It never clears a flag it did not set.** If `SleepDisabled` is already on when Vent asks, the helper refuses, says so, and the tab reports "This Mac cannot sleep at all - set with pmset, not by Vent" with the undo command as selectable text. A marker with no flag behind it is dropped, never acted on. The window that is left is the gap between two reads: clear the flag with `pmset` and set it again by hand between one read and the next, and Vent would still think that flag is its own.
...
- **`pmset disablesleep`.** When the system-wide `SleepDisabled` setting is 1 and the helper's marker does not claim it, the tab says so and offers `sudo pmset disablesleep 0` as selectable text with a copy button. Vent never clears a flag it did not set: undoing somebody else's is the user's own to do, and it needs root. Vent's own flag comes off with the switch that set it.
...
- [ ] `kill -9` Vent while the lid hold is on: `pmset -g` says SleepDisabled 0 within a second, because the helper clears it when the last connection holding it disconnects.
- [ ] With the app's switch on, run `ventctl awake lid on` and let it exit, then `ventctl awake lid off`: `pmset -g` still says SleepDisabled 1 both times, because the app is still holding it. Switch Keep Awake off and it says 0.
